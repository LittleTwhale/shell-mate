...
> 如果您的自然语言描述中**包含空格或换行**，请务必使用双引号 `""` 将其完整包裹，否则会被终端识别为多个参数而报错。
...
```bash
$ sm 帮我把本地的 README.md 传到远程服务器上

# 自动弹窗让用户补全占位符:
# ── 检测到缺失参数，请补充完整 ──
# 当前命令: scp README.md <Username>@<Server_IP>:
#
# 请输入 <Username>:
# > root
# 请输入 <Server_IP>:
...
# 填空完成后进入标准确认菜单
# ── 是否执行此命令? ──
# 命令: scp README.md root@192.168.1.100:
#   [y] 执行 (Execute)
```
